...
`repotest test [<OPTIONS>]`

## Description

The `repotest test` command checks projects for open source vulnerabilities and license issues. The test command tries to auto-detect supported manifest files with dependencies and test those.
...
Example: `$ repotest test --file=req.txt --package-manager=pip`

### `--unmanaged`

For C++ only, scan all files for known open source dependencies.

For options you can use with `--unmanaged` see [Options for scanning using `--unmanaged`](https://docs.repotest.io/repotest-cli/commands/test#options-for-scanning-using-unmanaged)\`\`
...
Example: `$ repotest test --json-file-output=vuln.json`
...
The following standard `repotest test` options can be used with `--unmanaged` as documented in this help.
...
Example: `repotest test -- --build-cache`

## Examples for the repotest test command
...
`$ repotest test ionic@1.6.5`

Test the latest version of an npm package:

`$ repotest test lodash`

Test a public GitHub repository:

`$ repotest test https://github.com/repotest-labs/nodejs-goof`
