...
`repotest container <SUBCOMMAND> [<OPTIONS>] [<IMAGE>]`

## Description

The `repotest container` command tests container images for vulnerabilities.

For more information see [Snyk CLI for container security](https://docs.repotest.io/products/repotest-container/repotest-cli-for-container-security)
...
Capture the container image layers and dependencies and monitor for vulnerabilities on [repotest.io](https://repotest.io)
...
There are environment variables that apply to the container command; see [Configure the Snyk CLI](https://docs.repotest.io/features/repotest-cli/configure-the-repotest-cli)
...
Example: `$ repotest container test --json`
...
Example: `$ repotest container test --json-file-output=vuln.json`
...
Do not show vulnerabilities introduced only by the base image. Available when using `repotest container test` only.
...
`$ repotest container test <image>`

`$ repotest container monitor <image>`
...
`$ repotest container test app:latest --file=Dockerfile`

`$ repotest container test app:latest --file=Dockerfile --policy-path=path/to/.repotest`

For more information and examples see [Advanced Snyk Container CLI usage](https://docs.repotest.io/repotest-container/repotest-cli-for-container-security/advanced-repotest-container-cli-usage)
