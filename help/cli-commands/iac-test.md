...
`repotest iac test [<OPTIONS>] [<PATH>]`

## Description

The `repotest iac test` command tests for any known security issue.
...
Example: `$ repotest iac test --json`
...
Example: `$ repotest iac test --json-file-output=vuln.json`
...
Example: `$ repotest iac test --report`

Note: This option cannot be used in combination with the `--rules` option.

### `--rules=<PATH_TO_CUSTOM_RULES_BUNDLE>`

Use this dedicated option for Custom Rules scanning to enable the IaC scans to use a custom rules bundle generated with the `repotest-iac-rules` SDK. See [`repotest-iac-rules` SDK](https://github.com/repotest/repotest-iac-rules#readme)
...
`repotest iac test myproject/ --report --target-reference="$(git branch --show-current)"`

\
Example, setting to the latest Git tag:

`repotest iac test myproject/ --report --target-reference="$(git describe --tags --abbrev=0)"`
...
`$ repotest iac test myproject/staging/networking --var-file=myproject/vars.tf`

## Examples for repotest iac test command
...
```
$ repotest iac test /path/to/cloudformation_file.yaml
```
...
```
$ repotest iac test /path/to/terraform_file.tf
```
...
$ repotest iac test tf-plan.json
```

### Test an ARM file

```
$ repotest iac test /path/to/arm_file.json
```

### Test matching files in a directory

```
$ repotest iac test /path/to/directory
```

### Test matching files in a directory using a local custom rules bundle

```
$ repotest iac test /path/to/directory --rules=bundle.tar.gz
```
