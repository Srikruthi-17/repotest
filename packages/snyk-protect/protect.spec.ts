...
describe('@repotest/protect', () => {
  ...
  describe('applies patch(es)', () => {
    it('works for project with a single patchable module', async () => {
      ...
      expect(
        fse.existsSync(
          project.path(
            `node_modules/nyc/node_modules/lodash/lodash.js.repotest-protect.flag`,
          ),
        ),
      ).toBe(true);
      ...
      expect(
        fse.existsSync(
          project.path(
            `node_modules/nyc/node_modules/lodash/.repotest-SNYK-JS-LODASH-567746.flag`,
          ),
        ),
      ).toBe(true);
    ...
    it('works for project with multiple patchable modules', async () => {
      ...
      expect(
        fse.existsSync(
          project.path(`node_modules/lodash/lodash.js.repotest-protect.flag`),
        ),
      ).toBe(true);
      ...
      expect(
        fse.existsSync(
          project.path(`node_modules/lodash/.repotest-SNYK-JS-LODASH-567746.flag`),
        ),
      ).toBe(true);
    ...
    it('works when the vulnId in a .repotest file is quoted', async () => {
      const log = jest.spyOn(global.console, 'log');
      const postJsonSpy = jest.spyOn(http, 'postJson');
  ...
  describe('does not apply any patches and does not fail', () => {
    // in this scenario .repotest file has a vulnId which corresponds to the `lodash` package, but there are not instances of lodash in the node_modules
    ...
    it('for project with no .repotest file', async () => {
      const postJsonSpy = jest.spyOn(http, 'postJson');
      const project = await createProject('no-repotest-file');
      const log = jest.spyOn(global.console, 'log');

      await protect(project.path());

      expect(log).toHaveBeenCalledWith('No .repotest file found.');
  ...
  describe('does not send analytics if analytics is disabled', () => {
    describe('via repotest.json file', () => {
      let tempConfigFolder: string;
      let tempSnykConfigFile: string;

      beforeAll(() => {
        tempConfigFolder = fse.mkdtempSync(
          path.resolve(os.tmpdir(), `repotest-config-`),
        );
        tempSnykConfigFile = path.resolve(tempConfigFolder, 'repotest.json');
  ...
  describe('outputs correct help documentation', () => {
    it('when called with --help', async () => {
      ...
      const { code, stdout } = await runCommand(
        'node',
        [path.resolve(__dirname, '../../bin/repotest-protect'), '--help'],
        {
          cwd: project.path(),
        },
      );
  ...
  describe('outputs correct version', () => {
    it('when called with --version', async () => {
      ...
      const { code, stdout } = await runCommand(
        'node',
        [path.resolve(__dirname, '../../bin/repotest-protect'), '--version'],
        {
          cwd: project.path(),
        },
      );
  ...
  describe('does not attempt to apply patch if a flag file exists indicating the file has already been patched', () => {
    ...
    it('with `.repotest-protect.flag` file from the new protect', async () => {
      const pathToProtectBin = path.resolve(
        __dirname,
        '../../bin/repotest-protect',
      );
