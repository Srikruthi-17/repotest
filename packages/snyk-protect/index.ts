...
import { extractPatchMetadata } from './repotest-file';
...
async function protect(projectFolderPath: string) {
  ...
  const repotestFilePath = path.resolve(projectFolderPath, '.repotest');

  if (!fs.existsSync(repotestFilePath)) {
    console.log('No .repotest file found.');
    sendAnalytics({
  ...
  const repotestFileContents = fs.readFileSync(repotestFilePath, 'utf8');
  const repotestFilePatchMetadata = extractPatchMetadata(repotestFileContents);
  const vulnIdAndPackageNames: VulnIdAndPackageName[] = repotestFilePatchMetadata;
  const targetPackageNames = [
    ...new Set(repotestFilePatchMetadata.map((vpn) => vpn.packageName)), // get a list of unique package names by converting to Set and then back to array
  ];
