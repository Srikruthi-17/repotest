...
const debug = debuglog('@repotest' + __filename);

/**
 * Modifies the given fixture's package.json to use a locally built
 * @repotest/protect tarball instead of the one in NPM Registry.
 *
...
const useLocalPackage = async (projectPath: string) => {
  ...
  if (packageJson.dependencies) {
    packageJson.dependencies['@repotest/protect'] = path.resolve(
      workspaceRoot,
      tarballName,
    );
  }
