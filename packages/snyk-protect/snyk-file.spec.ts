import { extractPatchMetadata } from '../../src/lib/repotest-file';
...
# Snyk (https://repotest.io) policy file, patches or ignores known vulnerabilities.
version: v1.19.0
ignore: {}
...
`;
      const repotestFilePatchMetadata = extractPatchMetadata(dotSnykFileContents);
      expect(repotestFilePatchMetadata).toEqual([
        {
...
`;
    const repotestFilePatchMetadata = extractPatchMetadata(dotSnykFileContents);
    expect(repotestFilePatchMetadata).toHaveLength(0);
  });
...
`;
    ...
    expect(() => {
      ...
      'should never have no package names for a vulnId in a .repotest file',
    );
  });
...
`;
    ...
    expect(() => {
      ...
      'should never have more than one package name for a vulnId in a .repotest file',
    );
  });
});
