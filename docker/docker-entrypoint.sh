...
OUTPUT_FILE=repotest-result.json
MONITOR_OUTPUT_FILE=repotest-monitor-result.json
ERROR_FILE=repotest-error.log
HTML_FILE=repotest_report.html
SNYK_COMMAND="$1"
SNYK_PARAMS="${@:2}"
ADDITIONAL_ENV=""
...
runCmdAsDockerUser "PATH=${PATH} repotest ${SNYK_COMMAND} --json ${SNYK_PARAMS} \
...
if [ -n "$MONITOR" ]; then
  ...
  runCmdAsDockerUser "PATH=$PATH repotest monitor --json ${SNYK_PARAMS} ${ADDITIONAL_ENV} > ${MONITOR_OUTPUT_FILE} 2>$ERROR_FILE"
fi

runCmdAsDockerUser "cat \"${OUTPUT_FILE}\" | repotest-to-html >> \"${PROJECT_PATH}/${PROJECT_FOLDER}/${HTML_FILE}\""
