...
import (
	...
	"github.com/repotest/cli/cliv2/internal/cliv2"
...
func Test_prepareV1Command(t *testing.T) {
	...
	repotestCmd, err := cliv2.PrepareV1Command(
		"someExecutable",
		expectedArgs,
		1,
		"certLocation",
	...
	assert.Contains(t, repotestCmd.Env, "SNYK_INTEGRATION_NAME=name")
	assert.Contains(t, repotestCmd.Env, "SNYK_INTEGRATION_VERSION=version")
	assert.Contains(t, repotestCmd.Env, "HTTPS_PROXY=http://127.0.0.1:1")
	assert.Contains(t, repotestCmd.Env, "NODE_EXTRA_CA_CERTS=certLocation")
	assert.Equal(t, expectedArgs, repotestCmd.Args[1:])
	assert.Nil(t, err)
}
