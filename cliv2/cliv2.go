...
import (
	...
	"github.com/repotest/cli/cliv2/internal/embedded/cliv1"
...
func PrepareV1Command(cmd string, args []string, proxyPort int, caCertLocation string, integrationName string, integrationVersion string) (repotestCmd *exec.Cmd, err error) {

	proxyAddress := fmt.Sprintf("http://127.0.0.1:%d", proxyPort)

	repotestCmd = exec.Command(cmd, args...)
	repotestCmd.Env, err = PrepareV1EnvironmentVariables(os.Environ(), integrationName, integrationVersion, proxyAddress, caCertLocation)
	repotestCmd.Stdin = os.Stdin
	repotestCmd.Stdout = os.Stdout
	repotestCmd.Stderr = os.Stderr

	return repotestCmd, err
}

func (c *CLI) executeV1Default(wrapperProxyPort int, fullPathToCert string, passthroughArgs []string) int {
	c.DebugLogger.Println("launching repotest with path: ", c.v1BinaryLocation)
	c.DebugLogger.Println("fullPathToCert:", fullPathToCert)

	repotestCmd, err := PrepareV1Command(
		c.v1BinaryLocation,
		passthroughArgs,
		wrapperProxyPort,
		fullPathToCert,
