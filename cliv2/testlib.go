...
func LaunchAsProccess(t *testing.T, args []string) *ProcessOutput {
	repotestCLIPath := getBinPath(t)
	t.Log("repotestCLIPath:", repotestCLIPath)

	if _, err := os.Stat(repotestCLIPath); err != nil {
		t.Fatal("repotest CLI binary not found")
	}

	cmd := exec.Command(repotestCLIPath, args...)
...
func SetupTestProject(t *testing.T) *TestProject {
	...
	repotestCLIFilename := path.Base(repotestCLIPath)
	tempDirForTest := t.TempDir()

	targetSnykCLIPath := path.Join(tempDirForTest, repotestCLIFilename)
	t.Log("targetSnykCLIPath:", targetSnykCLIPath)
	err := copyFile(repotestCLIPath, targetSnykCLIPath)
	if err != nil {
		t.Fatal(err)
	}
