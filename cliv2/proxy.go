...
import (
	...
	"github.com/repotest/cli/cliv2/internal/httpauth"
...
func NewWrapperProxy(insecureSkipVerify bool, cacheDirectory string, cliVersion string, debugLogger *log.Logger) (*WrapperProxy, error) {
	...
	certName := "repotest-embedded-proxy"
	...
	certFile, err := os.CreateTemp(tempDir, "repotest-cli-cert-*.crt")
	if err != nil {
	...
	proxy.OnRequest().DoFunc(func(r *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
		...
		// Manipulate Header (replace x-repotest-cli-version)
		existingValue := r.Header.Get("x-repotest-cli-version")
		if existingValue != "" {
			debugLogger.Printf("Replacing value of existing x-repotest-cli-version header (%s) with %s\n", existingValue, cliVersion)
			r.Header.Set("x-repotest-cli-version", cliVersion)
		}
