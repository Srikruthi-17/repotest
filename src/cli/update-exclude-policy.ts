...
import * as repotestPolicyLib from 'repotest-policy';
...
export default async (...args: MethodArgs): Promise<any> => {
  ...
  // Ensure that this update-exclude-policy command can only be runned when using `repotest iac update-exclude-policy`
  // Avoid `repotest update-exclude-policy` direct usage
  if (options.iac != true) {
  ...
  try {
    ...
    let policy: Policy;
    try {
      policy = await repotestPolicyLib.load();
    } catch (error) {
      if (error.code === 'ENOENT') {
        // policy file does not exist - create it
        policy = await repotestPolicyLib.create();
      } else {
        throw error;
      }
    }
    await updateExcludeInPolicy(policy, analysis, options);
    await repotestPolicyLib.save(policy);
  } catch (e) {
