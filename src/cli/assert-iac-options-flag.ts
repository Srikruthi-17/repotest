...
export class FlagError extends CustomError {
  constructor(key: string) {
    ...
    const msg = `Unsupported flag "${flag}" provided. Run repotest iac test --help for supported flags`;
    super(msg);
...
export class UnsupportedEntitlementFlagError extends CustomError {
  constructor(key: string, entitlementName: string) {
    ...
    this.strCode = getErrorStringCode(this.code);
    this.userMessage = `Flag "${flag}" is currently not supported for this org. To enable it, please contact repotest support.`;
  }
}
...
export class UnsupportedEntitlementCommandError extends CustomError {
  constructor(key: string, entitlementName: string) {
    ...
    this.code = IaCErrorCodes.UnsupportedEntitlementFlagError;
    this.strCode = getErrorStringCode(this.code);
    this.userMessage = `Command "${key}" is currently not supported for this org. To enable it, please contact repotest support.`;
  }
}

/**
 * Validates the command line flags passed to the repotest iac test
 * command. The current argument parsing is very permissive and
 * allows unknown flags to be provided without validation.
 *
 * For repotest iac we need to explicitly validate the flags to avoid
