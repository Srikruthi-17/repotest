...
import { SEVERITY, SEVERITIES } from '../../../../../../lib/repotest-test/common';
...
import {
  ...
  getTrees,
  parsePath,
} from '@repotest/cloud-config-parser';
  ...
  const formattedIssues = scanResult.violatedPolicies.map((policy) => {
    ...
    return {
      ...
      documentation: !isGeneratedByCustomRule
        ? `https://repotest.io/security-rules/${policy.publicId}`
        : undefined,
...
export class FailedToFormatResults extends CustomError {
  constructor(message?: string) {
    ...
    this.userMessage =
      'We failed printing the results, please contact support@repotest.io';
  }
}
