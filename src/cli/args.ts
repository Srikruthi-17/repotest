...
const DEBUG_DEFAULT_NAMESPACES = [
  ...
  'repotest:run-test',
  'repotest:prune',
  'repotest-nodejs-plugin',
  'repotest-gradle-plugin',
  'repotest-sbt-plugin',
  'repotest-mvn-plugin',
  'repotest-yarn-workspaces',
  'repotest-java-call-graph-builder',
];
...
export interface ArgsOptions {
  ...
  // (see the repotest-mvn-plugin or repotest-gradle-plugin)
...
export function args(rawArgv: string[]): Args {
  ...
  const debug = debugModule('repotest');
  ...
  // an argument to our command, like `repotest test package2test`
  ...
  // repotest [mode?] [command] [paths?] [options-double-dash]
  ...
  // alias `-v` to `repotest version`
  if (argv.version) {
    command = 'version';
  }

  if (!command || argv.help || command === 'help') {
    // bit of a song and dance to support `repotest -h` and `repotest help`
    ...
    // so that no argument gets deleted or ignored. This ensures `repotest --help [command]` and `repotest [command] --help` return the
    // specific help page instead of the generic one.
    // This change also covers the scenario of 'repotest [mode] [command] --help' and 'repotest --help [mode] [command]`.
    if (!argv._.length) {
  ...
  if (command && command.indexOf('config:') === 0) {
    ...
    // repotest.config('set', 'api=x')
    // repotest.config('get', 'api') // etc
