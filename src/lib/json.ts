...
function vulnerabilitiesToIacIssues(
  ...
  return vulnerabilities.map((v) => {
    ...
    return {
      ...
      publicId: v.rule.id,
      title: v.rule.title,
      references: ['TBD'], // missing in `repotest-iac-test` v0.3.0 will be added in a future release
      id: v.rule.id,
      isIgnored: v.ignored,
      iacDescription: {
        issue: v.rule.title,
      ...
      compliance: [['TBD']], // missing in `repotest-iac-test` v0.3.0 will be added in a future release
      description: v.rule.description,
    };
  });
}

// TODO: add correct mapping to our packageManger name (will probably be done in `repotest-iac-test`)
