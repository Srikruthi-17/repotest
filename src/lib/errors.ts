...
const defaultUserMessage =
  'Your test request could not be completed. Please run the command again with the `-d` flag and contact support@repotest.io with the contents of the output';

const repotestIacTestErrorsUserMessages = {
...
export function getErrorUserMessage(code: number): string {
  ...
  return repotestIacTestErrorsUserMessages[errorName];
}
