/*
  ...
  - repotest config file with key "endpoint" (including override with SNYK_CFG_ENDPOINT envvar!)
  - SNYK_API envvar
  - Snyk REST API had their own envvars to be set

  And API URL itself could (currently) point to multiple places
  - https://repotest.io/api/v1 (old default)
  - https://repotest.io/api
  - https://app.repotest.io/api
  - https://app.repotest.io/api/v1
  - https://api.repotest.io/v1

  For Snyk REST API it's a bit simpler:
  - https://api.repotest.io/rest


  There are also other URLs - one for the repotest auth command, one for Snyk Code Proxy
...
export function getBaseApiUrl(
  ...
  if (configDefinedApiUrl) {
    return validateUrlOrReturnDefault(
      configDefinedApiUrl,
      "'endpoint' config option. See 'repotest config' command. The value of 'endpoint' is currently set as",
      defaultBaseApiUrl,
    );
  }
...
export function getRestApiUrl(
  ...
  if (parsedBaseUrl.host?.startsWith('app.')) {
    // Rewrite app.repotest.io/ to api.repotest.io/rest
