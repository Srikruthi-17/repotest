...
const debug = baseDebug('repotest-yarn-workspaces');
import * as lockFileParser from 'repotest-nodejs-lockfile-parser';
import {
...
export async function processYarnWorkspaces(
  ...
  const result: MultiProjectResultCustom = {
    plugin: {
      name: 'repotest-nodejs-yarn-workspaces',
  ...
  for (const directory of Object.keys(yarnTargetFiles)) {
    ...
    try {
      ...
      const project: ScannedProjectCustom = {
        ...
        depTree: res as any,
        plugin: {
          name: 'repotest-nodejs-lockfile-parser',
          runtime: process.version,
        },
      };
