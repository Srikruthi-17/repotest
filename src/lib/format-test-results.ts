...
} from 'repotest-docker-plugin/dist';
...
function getDockerSuggestionText(options, config, baseImageRes): string {
  ...
  if (config && config.disableSuggestions !== 'true') {
    const optOutSuggestions =
      '\n\nTo remove this message in the future, please run `repotest config set disableSuggestions=true`';
    if (!options.file) {
      if (!baseImageRes) {
        dockerSuggestion +=
          chalk.bold.white(
            '\n\nSnyk wasn’t able to auto detect the base image, use `--file` option to get base image remediation advice.' +
              `\nExample: $ repotest container test ${options.path} --file=path/to/Dockerfile`,
          ) + optOutSuggestions;
      }
...
function getDockerRemediationDocsLink(dockerAdvice: string, config): string {
  ...
  return (
    ...
    chalk.white.underline(
      'https://docs.repotest.io/products/repotest-container/getting-around-the-repotest-container-ui/base-image-detection',
    )
  );
}
