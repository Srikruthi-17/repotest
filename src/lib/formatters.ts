...
export function formatSnykIacTestTestData(
  repotestIacTestScanResult: Results | undefined,
  projectName: string,
  orgName: string,
): IacTestData {
  const resultsBySeverity = formatSnykIacTestScanResultNewOutput(
    repotestIacTestScanResult,
  );
  ...
  const allFilesCount = countFiles(repotestIacTestScanResult);
  const filesWithIssuesCount = countFilesWithIssues(repotestIacTestScanResult);
...
function formatSnykIacTestScanResultNewOutput(
  ...
  if (repotestIacTestScanResult?.vulnerabilities) {
    repotestIacTestScanResult.vulnerabilities.forEach((vulnerability) => {
