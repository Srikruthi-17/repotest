...
const { SEVERITIES } = require('../repotest-test/common');
...
const errors = {
  ...
  auth: 'Unauthorized: please ensure you are logged in using `repotest auth`',
  oldrepotest:
  ...
  nodeModules:
    ...
    're-run your repotest command.',
  noApiToken:
    '%s requires an authenticated account. Please run `repotest auth` ' +
    'and try again.',
  ...
  policyFile:
    ...
    'directory with a .repotest file',
  idRequired: 'id is a required field for `repotest ignore`',
  unknownCommand: '%s\n\nRun `repotest --help` for a list of available commands.',
...
const codes = {
  ...
  OLD_DOTFILE_FORMAT: errors.oldrepotest,
