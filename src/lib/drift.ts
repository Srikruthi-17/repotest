...
import repotestLogoSVG from './assets/repotest-logo';
import repotestFaviconBase64 from './assets/repotest-favicon';
...
function rebrandHTMLOutput(data: string): string {
  ...
  data = data.replace(
    faviconReplaceRegex,
    `<link rel="shortcut icon" type="image/x-icon" href="${repotestFaviconBase64}" />`,
  );
  ...
  data = data.replace(
    logoReplaceRegex,
    `<div id="brand_logo">${repotestLogoSVG}</div>`,
  );
