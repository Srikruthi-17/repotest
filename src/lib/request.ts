...
const debug = debugModule('repotest:req');
const repotestDebug = debugModule('repotest');
...
function setupRequest(payload: Payload) {
  ...
  payload.headers['x-repotest-cli-version'] = versionNumber;
  ...
  if (body) {
    ...
    repotestDebug('sending request to:', payload.url);
    repotestDebug('request body size:', json.length);
    repotestDebug('gzipped request body size:', data.length);

    let callGraphLength: number | null = null;
    if (body.callGraph) {
      callGraphLength = JSON.stringify(body.callGraph).length;
      repotestDebug('call graph size:', callGraphLength);
    }
  ...
  if (proxyUri) {
    repotestDebug('using proxy:', proxyUri);
    bootstrap({
      environmentVariableNamespace: '',
    });
  } else {
    repotestDebug('not using proxy');
  }
