...
export function convertEngineToSarifResults(
  ...
  const tool: sarif.Tool = {
    driver: {
      ...
      version: getVersion(),
      informationUri:
        'https://docs.repotest.io/products/repotest-infrastructure-as-code',
...
function extractReportingDescriptor(
  ...
  results.vulnerabilities.forEach((vulnerability) => {
    ...
    const tags = ['security']; // switch to rules.labels once `repotest-iac-test` includes this info
    ...
    rules[vulnerability.rule.id] = {
      ...
      helpUri: `https://repotest.io/security-rules/${vulnerability.rule.id}`,
    };
  });
  return Object.values(rules);
}
