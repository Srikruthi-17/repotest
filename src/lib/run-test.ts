...
import { parsePackageString as moduleToObject } from 'repotest-module';
...
import { CallGraphError, CallGraph } from '@repotest/cli-interface/legacy/common';
...
const debug = debugModule('repotest:run-test');
...
async function parseRes(
  ...
  if (!options['ignore-policy']) {
    ...
    const policy = await repotest.policy.loadFromText(res.policy);
...
async function assembleRemotePayloads(root, options): Promise<Payload[]> {
  ...
  // options.vulnEndpoint is only used by `repotest protect` (i.e. local filesystem tests)
